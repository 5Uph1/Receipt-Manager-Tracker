...
export default function DashboardSidebar({
  ...
  const mobileNavClassName = (item: DashboardNavItem) =>
    item === activeItem
      ? "flex flex-col items-center gap-1 text-[#2563EB]"
      : "flex flex-col items-center gap-1 text-[#94A3B8]";

  const navItems = [
    {
      key: "dashboard" as DashboardNavItem,
      label: "Dashboard",
      href: "/dashboard",
      icon: (
        <svg
    ...
    {
      key: "receipt" as DashboardNavItem,
      label: "Receipts",
    ...
    {
      key: "insight" as DashboardNavItem,
      label: "Analytics",
  ...
  return (
    <>
      ...
      <nav
        className="fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around border-t border-[#E2E8F0] bg-white px-2 pb-safe pt-2 md:hidden"
      ...
      >
        {navItems.map((item) => (
          <Link
            key={item.key}
            href={item.href}
            className={mobileNavClassName(item.key)}
          >
            <div
              className={`rounded-xl p-2 transition-colors ${
                item.key === activeItem ? "bg-[#EFF6FF]" : ""
              }`}
            >
              {item.icon}
            </div>
